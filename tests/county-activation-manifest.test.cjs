...
assert.equal(report.summary.visibleActivationCount, 1);
assert.equal(report.summary.blockedCount, 9);
assert.equal(report.summary.unknownParcelFreshnessCount, 9);
assert.deepEqual(report.baseline.visibleCountyIds, ["dallas-county-dcad", "harris-county-tx"]);
...
assert(report.counties.filter((county) => county.countyId !== "harris-county-tx").every((county) => county.activationDecision === "do-not-activate"));
...
assert(report.counties.filter((county) => county.countyId !== "harris-county-tx").every((county) => county.controls.registrySelectable === false));
...
assert(report.counties.filter((county) => county.countyId !== "harris-county-tx").every((county) => county.blockers.length > 0));
...
assert.equal(report.summary.mapSearchCapableButBlockedCount, 9);
